...
namespace CodeGenerator_Project
{
    public static class clsGenerator
    {
        public static string clsDataAccessUtil()
        {
            ...
            sb.AppendLine("         public static string GetConnectionString(string name)");
            sb.AppendLine("         {");
            sb.AppendLine("             return ConfigurationManager.ConnectionStrings[name]?.ConnectionString ?? string.Empty;");
            sb.AppendLine("         }");
        ...
        public static string GenerateDataLayer(string tableName, List<(string ColumnName, string DataType, bool IsNullable)> columns)
        {
            ...
            // GetByID Method
            sb.AppendLine($"        public static bool GetByID(int ID, {string.Join(", ", columns.Select(c => $"ref {clsUtil.ConvertSqlTypeToCSharp(c.DataType, c.IsNullable)} {c.ColumnName}"))})");
            sb.AppendLine("        {");
            sb.AppendLine("            bool isFound = false;");
            sb.AppendLine("            using (SqlConnection connection = new SqlConnection(clsDataAccessUtil.GetConnectionString(\"MyDatabaseConnection\")))");
            sb.AppendLine("            {");
            ...
            {
                sb.AppendLine($"                        {column.ColumnName} = ({clsUtil.ConvertSqlTypeToCSharp(column.DataType, column.IsNullable)})reader[\"{column.ColumnName}\"];");
            }
            sb.AppendLine("                    }");
            ...
            // Delete Method
            sb.AppendLine($"        public static bool Delete({clsUtil.ConvertSqlTypeToCSharp(columns[0].DataType, columns[0].IsNullable)} ID)");
            sb.AppendLine("        {");
            ...
            // AddNew Method
            sb.AppendLine($"        public static {clsUtil.ConvertSqlTypeToCSharp(columns[0].DataType, columns[0].IsNullable)} AddNew({string.Join(", ", columns.Select(c => $"{clsUtil.ConvertSqlTypeToCSharp(c.DataType, c.IsNullable)} {c.ColumnName}"))})");
            sb.AppendLine("        {");
            sb.AppendLine($"            {clsUtil.ConvertSqlTypeToCSharp(columns[0].DataType, columns[0].IsNullable)} result = 0;");
            sb.AppendLine("            using (SqlConnection connection = new SqlConnection(clsDataAccessUtil.GetConnectionString(\"MyDatabaseConnection\")))");
            sb.AppendLine("            {");
            sb.AppendLine($"                string query = \"INSERT INTO {tableName} ({string.Join(", ", columns.Select(c => c.ColumnName))}) VALUES ({string.Join(", ", columns.Select(c => "@" + c.ColumnName))});\";");
            sb.AppendLine("                SqlCommand command = new SqlCommand(query, connection);");
            foreach (var column in columns)
            {
            ...
            sb.AppendLine("                {");
            sb.AppendLine("                    connection.Open();");
            sb.AppendLine($"                    result = ({clsUtil.ConvertSqlTypeToCSharp(columns[0].DataType, columns[0].IsNullable)})command.ExecuteScalar();");
            sb.AppendLine("                }");
            sb.AppendLine("                catch (Exception ex)");
            sb.AppendLine("                {");
            sb.AppendLine("                    clsDataAccessUtil.LogError(ex);");
        ...
        public static string GenerateBusinessLayer(string tableName, List<(string ColumnName, string DataType, bool IsNullable)> columns)
        {
            ...
            // Private constructor to initialize the object with found data
            ...
            // Save Method (Handles AddNew and Update)
            ...
            sb.AppendLine("        {");
            sb.AppendLine($"            this.{columns[0].ColumnName} = cls{tableName}Data.AddNew(this.{columns[0].ColumnName}, {string.Join(", this.", columns.Skip(1).Select(c => c.ColumnName))});");
            ...
            // Find Method
            sb.AppendLine($"        public static {className} Find(int ID)");
            sb.AppendLine("        {");
            foreach (var column in columns)
            {
            ...
            sb.AppendLine($"            if (cls{tableName}Data.GetByID(ID, {string.Join(", ", columns.Select(c => "ref " + c.ColumnName))}))");
            sb.AppendLine("            {");
